...
class ImageToRawMeshRequest(BaseModel):
    ...
    image_path: Optional[str] = Field(None, description="Path to the input image (for local files)")
    image_url: Optional[str] = Field(None, description="image url  (for files store on storage)")
    image_base64: Optional[str] = Field(None, description="Base64 encoded image data")
    image_file_id: Optional[str] = Field(None, description="File ID from upload endpoint")
...
class ImageToTexturedMeshRequest(BaseModel):
    ...
    output_format: str = Field("glb", description="Output mesh format")
    model_preference: str = Field(
        "hunyuan3d_image_to_textured_mesh", description="Model name for mesh generation"
    )
...
async def process_file_input(
    ...
    inputs = [file_path, base64_data, file_id, upload_file,img_url]
...
async def image_to_raw_mesh(mesh_request: ImageToRawMeshRequest,scheduler: MultiprocessModelScheduler = Depends(get_scheduler),):
    """
